...
3. SQL migration zinciri sıralıdır: `schema.sql` (yalnız tablolar) → `security_remediation.sql` → `_2_rpcs.sql` → `_3.sql` → `_4.sql` → `_5.sql` → `_6.sql` → `_7.sql` → `_8.sql`. `schema.sql`'deki superseded fonksiyon/policy tanımlarını yeniden uygulama. (Eski security_patch.sql, dual_balance_migration.sql, remove_transfer_balance.sql, fix_referral_and_packages.sql silindi — gerekirse git geçmişinde.)
