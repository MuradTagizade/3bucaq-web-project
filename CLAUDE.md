...
3. SQL migration zinciri sıralıdır ve `sql/` klasöründedir: `sql/schema.sql` (yalnız tablolar) → `sql/security_remediation.sql` → `_2_rpcs.sql` → `_3.sql` → ... → `_15.sql` (en güncel). Yeni migration dosyalarını da `sql/` içine ekle. `schema.sql`'deki superseded fonksiyon/policy tanımlarını yeniden uygulama. (Eski security_patch.sql, dual_balance_migration.sql, remove_transfer_balance.sql, fix_referral_and_packages.sql silindi — gerekirse git geçmişinde.)
