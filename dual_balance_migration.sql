-- ====================================================================
-- 3bucaq — Dual Balance System Migration
-- Run this script in the Supabase Dashboard SQL Editor.
--
-- Changes:
-- 1. transfer_funds: uses transfer_balance instead of balance
-- 2. buy_package: deducts from transfer_balance, adds to balance (locked)
-- 3. create_level_claim: bonus goes to transfer_balance (auto-approved)
-- 4. create_withdrawal: deducts from transfer_balance
-- 5. referral bonuses: go to transfer_balance
-- 6. NEW: deactivate_package RPC (balance → transfer_balance after lock)
-- ====================================================================


-- 1. TRANSFER FUNDS — uses transfer_balance
...
begin
  ...
  -- Check transfer_balance (not balance)
  if sender_profile.transfer_balance < parsed_amount then
    return json_build_object('success', false, 'error', 'Transfer balansı kifayət etmir');
  end if;
  ...
  -- Deduct from sender transfer_balance
  update public.profiles
    set transfer_balance = transfer_balance - parsed_amount
    where id = sender_id;

  -- Add to recipient transfer_balance
  update public.profiles
    set transfer_balance = transfer_balance + parsed_amount
...
-- 2. BUY PACKAGE — deduct from transfer_balance, add to balance (locked)
...
begin
  ...
  -- Check transfer_balance (not balance)
  if buyer_profile.transfer_balance < pkg_price then
    return json_build_object('success', false, 'error', 'Transfer balansı kifayət etmir');
  end if;
  ...
  -- Deduct from transfer_balance, add to balance (locked)
  update public.profiles
    set transfer_balance = transfer_balance - pkg_price,
  ...
  while upline_id is not null and current_depth <= 5 loop
    ...
    if exists (
      ...
      -- 2. USD Bonuses → transfer_balance (not balance)
      if current_depth = 1 then
        ...
        if coalesce((parent_profile.active_packages->>'pkg19')::boolean, false) = true
          ...
          update public.profiles
            set transfer_balance = transfer_balance + bonus_to_add
            where id = parent_profile.id;
        ...
        -- 1% Depth bonus (lines 2-5) → transfer_balance
...
-- 3. CREATE LEVEL CLAIM — bonus goes to transfer_balance, auto-approved
...
begin
  ...
  -- Deduct points, credit transfer_balance (not balance), update level
  update public.profiles
    set total_points = total_points - req_points,
        claimed_levels = claimed_levels || to_jsonb(claim_level),
        transfer_balance = transfer_balance + bonus_amt,
  ...
  -- Record level claim (auto-approved, always balance type now)
...
-- 4. CREATE WITHDRAWAL — deduct from transfer_balance
...
begin
  ...
  -- Check transfer_balance (not balance)
  if user_profile.transfer_balance < parsed_amount then
    return json_build_object('success', false, 'error', 'Transfer balansı kifayət etmir');
  end if;

  -- Deduct from transfer_balance
  update public.profiles
    set transfer_balance = transfer_balance - parsed_amount
    where id = user_id;
...
-- 5. NEW: DEACTIVATE PACKAGE — unlock balance → transfer_balance after lock period
...
begin
  ...
  -- Deactivate: move balance → transfer_balance
  update public.profiles
    set active_packages = jsonb_set(active_packages, array[pkg_id], 'false'::jsonb),
        balance = balance - pkg_price,
        transfer_balance = transfer_balance + pkg_price
    where id = user_id;
