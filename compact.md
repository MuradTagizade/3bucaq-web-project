...
* **`sql/security_remediation_15.sql` — TƏTBİQ EDİLDİ ✅ (psql, canlı DB; rollback-lı simulyasiya ilə tam doğrulandı):**
    ...
    * **`check_username_available(p_username)`** RPC (anon+auth grant): format (`^[A-Za-z]{3,20}$`) + case-insensitive mövcudluq → `{available, reason: empty|invalid|taken}`. Enumerasyon riski `check_referral_code` ilə eynidir (qəbul edilir).
...
* **Frontend (build ✅):**
    ...
    * `transfer/page.js`: alıcı sahəsi **kod VƏ ya username** (min uzunluq 6→**3**), placeholder `recipient_code_or_username_placeholder`.
...
* **Qərarlar:** username min **3**, maks **20**, yalnız hərf; unikallıq **case-insensitive** (transfer qarışıqlığı olmasın), display case qorunur. Telefon **DB sütunu toxunulmadı** (reversible; `updateUserProfile` whitelist-ində `phone` qalır amma UI göndərmir). Köhnə istifadəçilər username=null → yalnız user_code ilə tapılır (lansman sıfırlaması onsuz da gələcək).
