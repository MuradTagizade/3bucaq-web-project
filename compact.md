...
## 23. Son Sessiya (2026-07-07, davamı): İstifadəçi adı (username) sistemi + qeydiyyatdan telefon çıxarıldı (`sql/security_remediation_15.sql` ✅ CANLIDA)

İstifadəçi istəyi: qeydiyyatda telefon "şimdilik gerek yox — söndür"; istifadəçi qeydiyyatda **öz istifadəçi adını seçsin** (yalnız hərf, maks 20, rəqəm/simvol yox, böyük/kiçik hərf olar), yazılarkən **canlı yoxlansın** (boşdursa yaşıl, doludursa qırmızı), transferdə **username ilə də** pul göndərilə bilsin, qeydiyyatdan sonra istifadəçi **özü dəyişə bilməsin**.

* **`sql/security_remediation_15.sql` — TƏTBİQ EDİLDİ ✅ (psql, canlı DB; rollback-lı simulyasiya ilə tam doğrulandı):**
    * **`profiles.username`** sütunu (text) + **case-insensitive unique index** (`profiles_username_lower_uk` on `lower(username)`; null-lar fərqli sayılır → köhnə/admin istifadəçilər null ilə problemsiz). Yazıldığı kimi saxlanır, unikallıq `lower()` ilə (`John`=`john`).
    * **`check_username_available(p_username)`** RPC (anon+auth grant): format (`^[A-Za-z]{5,20}$`) + case-insensitive mövcudluq → `{available, reason: empty|invalid|taken}`. Enumerasyon riski `check_referral_code` ilə eynidir (qəbul edilir).
    * **`handle_new_user` + `create_profile_if_missing`** (`_4`-ü əvəz edir): username `raw_user_meta_data->>'username'`-dən alınır, format-guard (uyğun deyilsə null), unikallıq yoxlanır (çakışma → signup rollback, **orphan auth user yaranmaz**). user_code yenə də avtomatik.
    * **`check_profile_updates`** (`_10`-u əvəz edir, INVOKER, `revoke all` təkrar tətbiq): `username` **normal user üçün DƏYİŞMƏZ** (`display_login` modeli); admin yalnız `has_admin_perm('users')` ilə dəyişə bilər. (A) sistem-sütun bloku və bütün digər qorumalar bayt-bayt qorunub.
    * **`transfer_funds`** (`_3`-ü əvəz edir): alıcı **əvvəl `user_code` (dəqiq, upper), tapılmasa `username` (case-insensitive)** — determinist, ambiguity yox. `lookup_user_code` da eyni iki-addımlı axtarış + `username` qaytarır (self-transfer guard `user_code` ilə işləyir).
    * **Doğrulama (hamısı rollback-lı):** username sütun+index var; availability RPC (invalid/taken/available/empty); case-insensitive unikallıq rədd; transfer lowercase/UPPER username + user_code ilə → success, balans düzgün köçdü (sender −10, recipient +10); lookup 3 formada tapır, yoxu `exists:false`; **normal user öz username-ni dəyişə bilmir** (P0001 bloklandı), amma `full_name` dəyişə bilir (kontrol ✓); anon `transfer_funds` → 42501; anon `check_username_available` işləyir ✓.
* **Frontend (build ✅):**
    * `register/page.js`: **telefon sahəsi/state/validation tamamilə çıxarıldı**; yeni **username sahəsi** (AtSign ikon, girişdə hərf-dışı simvollar strip olunur, 400ms debounce canlı yoxlama, stale-guard): available→yaşıl border+mesaj, taken/invalid→qırmızı `error`, checking→boz. Submit-də format + son (yarış halına qarşı) availability yoxlaması; Supabase generic "Database error" → username-retry mesajına map olunur. `registerUser` metadata-ya `username` əlavə edir; telefon artıq göndərilmir.
    * `transfer/page.js`: alıcı sahəsi **kod VƏ ya username** (min uzunluq 6→**5**; username min 5), placeholder `recipient_code_or_username_placeholder`.
    * `personal-info/page.js`: **telefon sahəsi/state/handler/validation çıxarıldı** (4 payload-dan `phone` silindi); əlavə: **read-only kilidli "İstifadəçi adı" sahəsi** (varsa göstərilir, `Lock` ikon).
    * `AuthProvider.js`: `username` store-a map edildi (admin + user); `validators.js`: `validateUsernameCode` (kod qaytarır: required|length|chars) + `USERNAME_RE`; `database.js`: `checkUsernameAvailable` helper.
    * **i18n:** 10 yeni açar (`username_label/_placeholder/_hint/_checking/_available/_taken/_invalid/_required/_taken_retry`, `recipient_code_or_username_placeholder`) **5 dildə** (en/ru/tr/de/fr), paritet təsdiqləndi (hər faylda 10). Register label mövcud `username` açarını istifadə edir.
* **Admin username backfill (2026-07-07):** iki superadminin `username`-i (əvvəl null) `display_login`-ə bərabər dolduruldu — `admin@levelup.com → admin`, `mockadmin@levelup.com → mockadmin` (data-only UPDATE, kod yox). Yeni subadmin-lər (create-subadmin route) hələ username=null yaranır. Lansman sıfırlamasında admin username-lərini qoru.
* **Register layout (2026-07-07):** username + email eyni `.formRow`-da yan-yana (username solda, email sağda; mobil ≤480px-də sıra ilə stack); `username_placeholder` qısaldıldı (label-stili, yarım-en sahədə kəsilməsin).
* **Qərarlar:** username min **5**, maks **20**, yalnız hərf; unikallıq **case-insensitive** (transfer qarışıqlığı olmasın), display case qorunur. Telefon **DB sütunu toxunulmadı** (reversible; `updateUserProfile` whitelist-ində `phone` qalır amma UI göndərmir). Köhnə istifadəçilər username=null → yalnız user_code ilə tapılır (lansman sıfırlaması onsuz da gələcək).
* **Deploy TAMAMLANDI ✅:** SQL `_15` canlıya tətbiq olundu; frontend `main`-ə **push + Vercel production deploy** edildi — commit-lər: **`011f4c4`** (username sistemi + telefon çıxarıldı), **`29f400a`** (min 5), **`a23c7ae`** (username+email yan-yana + admin backfill). Working tree təmiz (yalnız `.claude/settings.json` = harness icazələri, işə aid deyil). Aktiv SQL zənciri: `sql/`-də **_15-ə qədər**. §20-dəki LANSMAN SIFIRLAMA PLANI hələ də gözləyir.

## 24. Son Sessiya (2026-07-07, davamı 2): İstifadəçi adı formatı (rəqəm/kiçik) + user_code istifadəçiyə gizli + referal linki paketdən AYRILDI (`sql/security_remediation_16.sql` ✅ CANLIDA)

İstifadəçi istəkləri: (1) istifadəçi tərəfdə **user_code (ID) göstərilməsin**, yalnız **istifadəçi adı** kifayətdir; (2) istifadəçi adı **rəqəm də** qəbul etsin, **yalnız kiçik hərflər** (böyük hərflər avtomatik kiçildilir); (3) level açılanda paketlər sönür (§22) və hotbed #19 də sönür deyə **referal kodu/linki verilə bilmirdi** — bunu düzəlt, **paket deaktiv olanda referal linki etkilenməsin**.

* **`sql/security_remediation_16.sql` — TƏTBİQ EDİLDİ ✅ (psql, canlı DB; verify-lı):** `_15` (username RPC-ləri), `_4` (check_referral_code) və `_3`-dəki get_my_referral_tree tanımlarını əvəz edir. Part 6 whitelist grant qorunur.
    * **İstifadəçi adı formatı `^[A-Za-z]{5,20}$` → `^[a-z0-9]{5,20}$`** (rəqəm əlavə, yalnız kiçik). `check_username_available`, `handle_new_user`, `create_profile_if_missing` — hamısında `lower(trim(...))` normalizə + yeni regex. Böyük hərf gələrsə kiçildilir (rədd yox). Case-insensitive unikallıq index (`profiles_username_lower_uk`) dəyişmədi.
    * **Referal PAKETDƏN AYRILDI:** `check_referral_code` artıq `reason:'inactive'` QAYTARMIR — kod mövcuddursa **həmişə `valid:true`**. `handle_new_user`/`create_profile_if_missing`-də `referred_by` referrer-in aktiv paketinə **baxmadan** set edilir. (Beləliklə level açıb paketləri sönən istifadəçinin referal linki işləməyə davam edir.)
    * **`get_my_referral_tree`** çıxışa `username` əlavə etdi (UI user_code yerinə username göstərir).
    * **Doğrulama (canlı):** `abc12`/`user9` available (rəqəm keçir); `ABC12`→available (lower normalizə); `ab1`/`ab-cd`→invalid; paketi `{}` olan referrer `REF57322`→**`valid:true`** (əvvəl inactive idi); yox kod→valid:false. ✓
    * **QEYD (dəyişməz saxlanıldı — istifadəçi təsdiqi):** referal **QAZANCI** (bonus paylanması) hələ də `buy_package`-da "upline-ın ≥1 aktiv paketi olmalı" qaydası ilədir (§12.4). Yəni link işləyir, amma paketi sönən referrer **yenidən paket almayana qədər bonus qazana bilmir**. Yalnız link tələbi düzəldildi.
* **Frontend (build ✅):**
    * **user_code istifadəçiyə gizli:** `personal-info`-da ayrıca "İstifadəçi Kodu" sahəsi silindi → indi **istifadəçi adı** sahəsi (kopyalana bilən; username yoxdursa köhnə istifadəçilər üçün user_code fallback + uyğun label). `subscribers` referal siyahısı (masaüstü sətir + mobil kart + detal modal + avatar hərfləri) artıq `username || userCode` göstərir. `dashboard` welcome adı `fullName || username || userCode`.
    * **Referal linki həmişə görünür:** `dashboard` refRow, `subscribers` referal kartı, `Sidebar`, `SlideUpMenu` — bütün "🔒 kilidli / paket alın" halları silindi; `hasActivePackage` gating referal üçün çıxarıldı (`layout.js` prop hələ ötürür, komponentlər istifadə etmir — zərərsiz). Köhnə `next/link` import subscribers-dən çıxarıldı.
